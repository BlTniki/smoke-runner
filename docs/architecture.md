...
`getUpdates` поддерживается Telegram Bot API. Foreground-переходы отправляют
свежий экран вниз чата и удаляют предыдущий, а фоновое обновление текущего
dashboard использует `editMessageText` и inline keyboard
...
4. Бот заменяет текущий экран свежим сообщением внизу чата и показывает
   шаблонную поддержку.
...
У пользователя хранится один актуальный `message_id` основного экрана. Каждый
foreground-переход сначала отправляет новый экран последним сообщением, сохраняет
его `message_id`, затем по возможности удаляет предыдущий. Так интерфейс не
уезжает вверх, но в чате по-прежнему остаётся один актуальный экран.

- `screen_kind=dashboard` разрешает пятиминутное автообновление.
- После отдельного milestone или отчёта scheduler снова отправляет dashboard
  последним и переводит автообновление на новый `message_id`.
